...
export default function BusinessProfile() {
  ...
  const [formData, setFormData] = useState({
    ...
    numberOfPlants: (user as any)?.numberOfPlants || '',
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600 mt-1">Manage your business information</p>
        </div>
        {!isEditing ? (
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col md:flex-row items-center md:items-start space-y-4 md:space-y-0 md:space-x-6">
            ...
            <div className="flex-1 text-center md:text-left">
              <h2 className="text-2xl font-bold text-purple-900">{formData.name}</h2>
              <p className="text-purple-700 font-medium">{formData.company}</p>
              <p className="text-purple-600 text-sm">{formData.email}</p>
              <div className="mt-2 inline-block px-3 py-1 bg-purple-200 text-purple-900 rounded-full text-xs font-semibold">
                Business Account
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              {isEditing ? (
                ...
                <p className="text-lg font-semibold text-slate-900 capitalize">{formData.industryType}</p>
              )}
            </div>

            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Number of Plants</label>
              {isEditing ? (
                <Input
                  type="number"
                  value={formData.numberOfPlants}
                  onChange={(e) => setFormData({ ...formData, numberOfPlants: e.target.value })}
                  placeholder="Enter number of plants"
                />
              ) : (
                <p className="text-lg font-semibold text-slate-900">{formData.numberOfPlants}</p>
              )}
            </div>
